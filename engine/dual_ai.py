...
class DualAI:
    def __init__(self):
        ...
        self.functions = {
            ...
            'open_multiple': self._open_multiple,
            ...
            'start_continuous_listen': self._start_continuous_listen,
            'stop_continuous_listen': self._stop_continuous_listen,
            'continuous_listen_status': self._get_continuous_listen_status,
        }
    ...
    def execute(self, query):
        try:
            ...
            if query_clean == 'start continuous listen':
                return self._start_continuous_listen()
            elif query_clean == 'stop continuous listen':
                return self._stop_continuous_listen()
            elif query_clean == 'continuous listen status':
                return self._get_continuous_listen_status()
            ...
            if query.lower().startswith('open '):
                ...
                if ' and ' in content or ',' in content:
                    items = re.split(r'\s+and\s+|,\s*', content)
                    items = [item.strip() for item in items if item.strip()]
                    if len(items) > 1:
                        return self._open_multiple(items)
    ...
    def understand_natural_speech(self, query):
        ...
        mappings = {
            ...
            'volume_up': ['volume up', 'louder', 'increase volume', 'make it louder', 'turn up sound'],
            ...
            'weather': ['weather', 'weather report', 'how is weather'],
            ...
            'get_weather': ['weather', 'weather report'],
    ...
    def _is_question(self, query):
        question_words = ['who', 'what', 'when', 'where', 'why', 'how', 'switch to gemini', 'switch to groq']
    ...
    def _get_response(self, func, result):
        responses = {
            ...
            'weather': "Weather information retrieved.",
            'news': "News opened.",
    ...
    def _get_weather(self, city="Delhi"):
        try:
            return f"Weather information for {city} - Please check your weather app for current conditions"
        except:
    ...
    def _open_multiple(self, items):
        try:
            opened_items = []
            failed_items = []
            
            # App and website mappings
            app_commands = {
                'notepad': 'notepad',
                'calculator': 'calc',
                'chrome': 'start chrome',
            ...
            for item in items:
                ...
                # Try to open as app first
                if item in app_commands:
                    try:
                        subprocess.run(app_commands[item], shell=True)
                ...
                if not success and item in website_urls:
                    try:
                        subprocess.run(f'start chrome {website_urls[item]}', shell=True)
                        opened_items.append(item)
                        success = True
                        time.sleep(0.5)
                    except:
                ...
                # Try generic website opening
                if not success and ('.' in item or 'www' in item):
                    try:
                        url = item if item.startswith('http') else f'https://www.{item}'
            ...
            result = f"Opened: {', '.join(opened_items)}"
            if failed_items:
                result += f" | Failed: {', '.join(failed_items)}"
            
            return result
            
        except Exception as e:
            return f"Failed to open multiple items: {str(e)}"
...
Max 2 lines each.'''
    ...
    def _start_continuous_listen(self):
        try:
                ...
                while self.continuous_active:
                    try:
                        ...
                        if text and len(text.strip()) > 2:
                            ...
                            print(f"Continuous: {text} -> {response}")
            ...
            self.continuous_listener = threading.Thread(target=continuous_loop, daemon=True)
    ...
    def _stop_continuous_listen(self):
        try:
            ...
            if hasattr(self, 'continuous_listener'):
                self.continuous_listener = None
            return "Continuous listening stopped"
        except Exception as e:
            return f"Error stopping continuous listening: {str(e)}"
    
    def _get_continuous_listen_status(self):
        try:
            if hasattr(self, 'continuous_active') and self.continuous_active:
